...
// Fetch the payment details from Razorpay
        ...
        if (payment.status === 'captured') {
            const orderPromise = OrderModel.update({status: "SUCCESS"}, {where: {orderId: order_id}});
            const userPromise = UserModel.update({isPremium: true}, {where: {id: req.user.id}});
            await Promise.all([orderPromise, userPromise]);
            res.status(200).json({success: true, message: "Payment successful and premium status updated"});
        } else {
            await OrderModel.update({status: "FAILED"}, {where: {orderId: order_id}});
